...
sub index {
  ...
  my $index = CPAN::AnyDBM::Index->new(%wanted, %args, log_dir => $log_dir);
  $index->index() or do {
...
sub query {
  my ($self, %args) = @_;
  require CPAN::AnyDBM::Search;
  my %wanted = map {$_ => $self->{$_}} qw(max_results CPAN db_name db_dir meta_obj);
  my $query = CPAN::AnyDBM::Search->new(%wanted, %args);
...
CPAN::AnyDBM - Faster and concurrent than CPAN::SQLite

=head1 SYNOPSIS

  my $obj = CPAN::AnyDBM->new(CPAN => '/path/to/CPAN');
  $obj->index(setup => 1);
...
CPAN::AnyDBM overcomes these problems.

One begins by creating the object as

  my $obj = CPAN::AnyDBM->new(%args);
...
Using CPAN::AnyDBM, what happens is that a request for information
through CPAN.pm, such as

  cpan> a ANDK

will cause a query to the AnyDBM database to be made.
...
L<AnyDBM> for the new nosql database backend.

L<CPAN::AnyDBM::Index>, for setting up and maintaining
the database, and L<CPAN::AnyDBM::Search> for an
interface to querying the database. Some details
of the interaction with L<CPAN> is available from
L<CPAN::AnyDBM::META>. See also the L<cpandb> script for a
command-line interface to the
indexing and querying of the database.

See L<https://github.com/rurban/CPAN-AnyDBM/>.
...
L<http://annocpan.org/dist/CPAN-AnyDBM>

=item * CPAN::Forum: Discussion forum

L<http:///www.cpanforum.com/dist/CPAN-AnyDBM>

=item * CPAN Ratings

L<http://cpanratings.perl.org/d/CPAN-AnyDBM>

=item * RT: CPAN's request tracker

L<http://rt.cpan.org/NoAuth/Bugs.html?Dist=CPAN-AnyDBM>

=item * Search CPAN

L<http://search.cpan.org/dist/CPAN-AnyDBM>

=back

=head1 BUGS

At this time, CPAN::AnyDBM keeps information contained only
...
This behaviour differs from that of L<CPAN> without CPAN::AnyDBM.
