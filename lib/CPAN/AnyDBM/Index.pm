...
sub index {
    ...
    if ($log_cleanup and $log_cleanup =~ /^\d+$/) {
      if (opendir(my $DIR, $self->{'log_dir'})) {
        my @files = grep { /cpan_dbm_log\./ } readdir $DIR;
        closedir $DIR;
...
sub fetch_info {
  ...
  my $info = CPAN::AnyDBM::Info->new(%wanted);
...
sub state {
  ...
  my $state = CPAN::AnyDBM::State->new(%wanted);
...
sub populate {
  ...
  my $db = CPAN::AnyDBM::Populate->new(%wanted);
...
CPAN::AnyDBM::Index - set up or update database tables.

=head1 SYNOPSIS

 my $index = CPAN::AnyDBM::Index->new(setup => 1);
 $index->index();
...
CPAN and ppm indices. The creation of the object
 ...
 my $index = CPAN::AnyDBM::Index->new(%args);
...
F<cpan_dbm_log.dddddddddd>, where the extension
...
at the beginning of L<CPAN::AnyDBM::Index>. If you are
...
method of L<CPAN::AnyDBM::Index> is specified,
this will get information on the state of the database
through L<CPAN::AnyDBM::State>.
...
the database, through L<CPAN::AnyDBM::Populate>,
...
L<CPAN::AnyDBM::Info>, L<CPAN::AnyDBM::State>, 
L<CPAN::AnyDBM::Populate>,
and L<CPAN::AnyDBM::Util>.
Development takes place on the CPAN-AnyDBM project
