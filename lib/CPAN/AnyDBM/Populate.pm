...
use CPAN::AnyDBM::Util qw($table_id has_hash_data print_debug);
use CPAN::AnyDBM::DBI::Index;
...
sub new {
  ...
  foreach my $table (@tables) {
    my $obj = $index->{$table};
    die "Please supply a CPAN::AnyDBM::Index::$table object"
      unless ($obj and ref($obj) eq "CPAN::AnyDBM::Index::$table");
  }
  my $state = $args{state};
  unless ($setup) {
    die "Please supply a CPAN::AnyDBM::State object"
      unless ($state and ref($state) eq 'CPAN::AnyDBM::State');
  }
  my $cdbi = CPAN::AnyDBM::DBI::Index->new(%args);
...
sub create_objs {
  ...
  foreach my $table (@tables) {
    ...
    if ($index and ref($index) eq "CPAN::AnyDBM::Index::$table") {
      my $info = $index->{info};
      if ($table ne 'info') {
...
package CPAN::AnyDBM::Populate::auths;
use parent 'CPAN::AnyDBM::Populate';
use CPAN::AnyDBM::Util qw(has_hash_data print_debug);
...
sub new {
  ...
  die "No dbi object available"
    unless ($cdbi and ref($cdbi) eq 'CPAN::AnyDBM::DBI::Index::auths');
  my $self = {
              info => $info,
              insert => {},
              update => {},
...
package CPAN::AnyDBM::Populate::dists;
use parent 'CPAN::AnyDBM::Populate';
use CPAN::AnyDBM::Util qw(has_hash_data print_debug);
...
sub new {
  ...
  my $cdbi = $args{cdbi};
  die "No dbi object available"
    unless ($cdbi and ref($cdbi) eq 'CPAN::AnyDBM::DBI::Index::dists');
  my $self = {
              info => $info,
              insert => {},
              update => {},
...
package CPAN::AnyDBM::Populate::mods;
use parent 'CPAN::AnyDBM::Populate';
use CPAN::AnyDBM::Util qw(has_hash_data print_debug);
...
sub new {
  ...
  my $cdbi = $args{cdbi};
  die "No dbi object available"
    unless ($cdbi and ref($cdbi) eq 'CPAN::AnyDBM::DBI::Index::mods');
  my $self = {
              info => $info,
              insert => {},
              update => {},
...
package CPAN::AnyDBM::Populate::chaps;
use parent 'CPAN::AnyDBM::Populate';
use CPAN::AnyDBM::Util qw(has_hash_data print_debug);
...
sub new {
  ...
  my $cdbi = $args{cdbi};
  die "No dbi object available"
    unless ($cdbi and ref($cdbi) eq 'CPAN::AnyDBM::DBI::Index::chaps');
  my $self = {
              obj => {},
              cdbi => $cdbi,
              error_msg => '',
...
package CPAN::AnyDBM::Populate::info;
use parent 'CPAN::AnyDBM::Populate';
use CPAN::AnyDBM::Util qw(has_hash_data print_debug);
...
sub new {
  ...
  my $cdbi = $args{cdbi};
  die "No dbi object available"
    unless ($cdbi and ref($cdbi) eq 'CPAN::AnyDBM::DBI::Index::info');
  my $self = {
              obj => {},
              cdbi => $cdbi,
              error_msg => '',
...
CPAN::AnyDBM::Populate - create and populate database tables
...
I<CPAN::AnyDBM::Info> and the
state information from I<CPAN::AnyDBM::State>. It does
...
C<CPAN::AnyDBM::Index> object; existing tables will be
dropped.
